...
class InstructionBlockController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    // debug button
    fileprivate var nextLevelButton = UIButton(frame: CGRect(x: 500, y: 50, width: 100, height: 50))
    ...
    fileprivate func processInstruction(_ col: Int, name: String, values: [Int]) -> String {
        if (col == 0) {
            return name
